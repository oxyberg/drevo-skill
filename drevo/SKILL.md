...
- Для проверки прямой линии или всех предков выбранного человека прочитать [generation-audit.md](references/generation-audit.md) и собрать исходную таблицу через `scripts/assemble_generation_audit.py`.
...
- Финальный аудит писать в доказательном, а не машинном формате: `Итог`, четырёхколоночная проверка каждого звена, только нужные содержательные разделы, решение по дереву и следующий приоритет. Не оставлять разделы о параметрах сборки или покрытии поколений.
- Перед завершением запускать `scripts/validate_generation_audit.py <отчёт>` и исправлять все ошибки формата.
...
- Перед бисекцией проверить границы, признак сортировки и надёжность решения по средней точке. Фиксировать проверенные опорные точки и оставшийся диапазон.
- Не использовать бисекцию как доказательство отсутствия, если порядок нарушен, есть пропуски или искомый признак может встречаться в нескольких частях. После сужения последовательно прочитать непрерывный целевой диапазон.

## Консолидировать итерации исследования
